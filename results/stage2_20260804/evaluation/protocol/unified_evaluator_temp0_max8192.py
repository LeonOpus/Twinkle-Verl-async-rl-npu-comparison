...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--reward-path', required=True)
...
def main() -> None:
    args = parse_args()
    if args.temperature != 0.0 or args.max_tokens != 8192:
        raise ValueError('stage-2 unified evaluation requires temperature=0 and max_tokens=8192')
    ...
    reward_spec = importlib.util.spec_from_file_location(
        'owner_gsm8k_accuracy_reward', args.reward_path
    )
    if reward_spec is None or reward_spec.loader is None:
        raise RuntimeError(f'cannot load owner reward: {args.reward_path}')
    ...
    with predictions_path.open('w', encoding='utf-8') as stream:
        for offset in range(0, len(prompts), args.batch_size):
            ...
            rewards = [
                compute_score(
                    'openai/gsm8k',
                    last_assistant_content(trajectory),
                    ground_truth(source),
                )
                for source, trajectory in zip(sources, evaluated)
            ]
