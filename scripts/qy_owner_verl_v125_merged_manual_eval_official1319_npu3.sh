...
EVAL_EXP=qy_owner_verl_v125_merged_manual_eval_official1319_npu3

MODEL=$ROOT/qy_owner_experiments_20260730/$TRAIN_EXP/merged_hf_v125_for_eval
ADAPTER=
...
if test "$GEN_RC" != 0; then
    ...
    echo "RESULT: VERL_V125_MANUAL_EVAL_GENERATION_FAIL_RC=$GEN_RC"
    exit "$GEN_RC"
fi
...
print("RESULT: VERL_V125_MANUAL_EVAL_SCORING_PASS")
PY_SCORE
...
if test "$SCORE_RC" = 0; then
    echo "RESULT: OWNER_VERL_V125_MANUAL_EVAL_PASS"
else
    echo "RESULT: OWNER_VERL_V125_MANUAL_EVAL_FAIL_RC=$SCORE_RC"
fi
